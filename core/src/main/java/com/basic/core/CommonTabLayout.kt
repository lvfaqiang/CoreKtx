...
import com.basic.core.tablayout.utils.FragmentChangeManager
import com.basic.core.tablayout.utils.UnreadMsgUtils.show
import com.basic.core.tablayout.widget.MsgView
import java.util.*

/** 没有继承HorizontalScrollView不能滑动,对于ViewPager无依赖  */
open class CommonTabLayout @JvmOverloads constructor(
    context: Context,
    ...
    private fun obtainAttributes(
        ...
        mIndicatorHeight = ta.getDimension(
            ...
            dp2px((if (mIndicatorStyle == STYLE_TRIANGLE) 4 else (if (mIndicatorStyle == STYLE_BLOCK) -1 else 2).toFloat()) as Float).toFloat()
        )
        mIndicatorWidth = ta.getDimension(
            R.styleable.CommonTabLayout_tl_indicator_width,
            dp2px((if (mIndicatorStyle == STYLE_TRIANGLE) 10 else -1.toFloat()) as Float).toFloat()
        )
        mIndicatorCornerRadius = ta.getDimension(
            R.styleable.CommonTabLayout_tl_indicator_corner_radius,
            dp2px((if (mIndicatorStyle == STYLE_BLOCK) -1 else 0.toFloat()) as Float).toFloat()
        )
        ...
        indicatorMarginTop = ta.getDimension(
            ...
            dp2px((if (mIndicatorStyle == STYLE_BLOCK) 7 else 0.toFloat()) as Float).toFloat()
        )
    ...
    fun notifyDataSetChanged() {
        ...
        for (i in 0 until tabCount) {
            tabView = if (mIconGravity == Gravity.START) {
    ...
    private fun addTab(position: Int, tabView: View) {
        ...
        tabView.setOnClickListener { v ->
            ...
            if (System.currentTimeMillis() - lastClickTime < 1000 && selectPosition == position) {
                if (mListener != null) {
                    mListener!!.onDoubleTabSelect(position)
                }
            }
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
     ...
     */
    fun showMsg(position: Int, num: Int, moreType: String?) {
        ...
        if (tipView != null) {
            ...
            if (!mIconVisible) {
                ...
                setMsgMargin(
                    position, 0f,
                    (if (mIconGravity == Gravity.LEFT || mIconGravity == Gravity.RIGHT) 4f else 0f)
                )
            }
    ...
    fun showMsg(
        ...
        val tipView =
            tabView.findViewById<View>(R.id.rtv_msg_tip) as MsgView
        tipView.textSize = unTextSize
        if (tipView != null) {
            show(tipView, num, moreType, wh)
            if (mInitSetMap[position] != null && mInitSetMap[position]!!) {
                return
            }
            if (!mIconVisible) {
        ...
        if (tipView != null) {
            ...
            if (mIconGravity == Gravity.TOP || mIconGravity == Gravity.BOTTOM) {
                ...
                lp.topMargin = if (mHeight > 0) (mHeight - Math.max(
                    textHeight,
                    iconH
                )).toInt() / 2 - dp2px(bottomPadding) else dp2px(bottomPadding)
            }
