...
object Decimal {
    ...
    fun maxKeepTwoDecimal(number: String?): String {
        return optionDecimal(number, 2, "#", BigDecimal.ROUND_HALF_UP)
    }
    ...
    fun maxKeepTwoDecimalDown(number: String?): String {
        return optionDecimal(number, 2, "#", BigDecimal.ROUND_DOWN)
    }
    ...
    fun keepTwoDecimal(number: String?): String {
        return optionDecimal(number, 2, "0", BigDecimal.ROUND_HALF_UP)
    }
    ...
    fun keepTwoDecimalDown(number: String?): String {
        return optionDecimal(number, 2, "0", BigDecimal.ROUND_DOWN)
    }
    ...
    fun keepDecimalRangeDown(value: String?, start: Int, end: Int): String? {
        val result =
            maxKeepDecimalDown(value, end)
        if (result.contains(".")) {
            val arrays = result.split(".")
            if (arrays[1].length >= start) {
                return result
            } else {
                return keepDecimalDown(
                    result,
                    start
                )
            }
    ...
    private fun optionDecimal(number: String?, count: Int, symbol: String, mode: Int): String {
        ...
        val stringBuilder = StringBuilder("#$symbol.")
        for (i in 0 until count) {
            stringBuilder.append(symbol)
        }
        val df = DecimalFormat(stringBuilder.toString())
        val bigDecimal: BigDecimal = try {
            if (mode != -1) {
                BigDecimal(num).setScale(count, mode)
            } else {
                BigDecimal(num)
            }
        } catch (e: Exception) {
